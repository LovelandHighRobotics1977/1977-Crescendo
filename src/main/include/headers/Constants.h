...
namespace Drivetrain {
	namespace Measurements {
		...
		namespace Circle {
			static const auto Radius = units::meter_t{std::hypot(((Frame::length/2)-Wheel::offset).value(), ((Frame::length/2)-Wheel::offset).value()) / 39.375};
	...
	namespace Movement {
		namespace Maximum {
			namespace Linear {
				...
				static const auto Velocity = units::meters_per_second_t{((((2 * Measurements::Wheel::radius.value() * M_PI) * (Swerve::Motor::Drive::max_rpm / Swerve::Motor::Drive::gear_ratio)))) / 60};
